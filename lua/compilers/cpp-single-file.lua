...
local option_value = "easyvim_cpp_current_file"

local function run_current_cpp_file()
	...
	local output_dir = utils.os_path(cwd .. "/bin-single-cpp/")
	local output = utils.os_path(cwd .. "/bin-single-cpp/program")
...
function M.apply()
	...
	if utils.easyvim_cpp_single_file_patched then
		return
	end

	utils.easyvim_cpp_single_file_patched = true
	...
	utils.require_language = function(filetype)
		...
		if filetype ~= "cpp" or not language then
			return language
		end
		...
		if not language.easyvim_cpp_single_file_action_patched then
			language.easyvim_cpp_single_file_action_patched = true
			local default_action = language.action
			language.action = function(selected_option)
				if selected_option == option_value then
					run_current_cpp_file()
					return
				end

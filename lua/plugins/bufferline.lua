...
return {
    ...
    config = function()
        ...
        local function close_all_other_buffers()
            -- Get information for all listed buffers
            local listed_buffers = vim.fn.getbufinfo({ buflisted = 1 })
            local current_bufnr = vim.api.nvim_get_current_buf()

            for _, buf in ipairs(listed_buffers) do
                local bufnr = buf.bufnr
                -- Do not close the currently active buffer
        ...
        -- Keymap: Press <Leader>a to trigger (you can choose any keymap)
        vim.keymap.set("n", "<leader>X", close_all_other_buffers, { desc = "Close all buffers except the current one" })
    end,
}
